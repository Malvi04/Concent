...
<!-- SweetAlert -->
	<script src="../files/assets/js/sweetalert2.js"></script>

<!-- Row Click Sweet Alert-->
...
$( document ).ready(function() {
    ...
    $( ".tblRows" ).click(function() {
        var row_data1 = $(this).attr("data1");
		var row_data2 = $(this).attr("data2");
		var row_data3 = $(this).attr("data3");
		var row_data4 = $(this).attr("data4");
		var row_data5 = $(this).attr("data5");
		var row_data6 = $(this).attr("data6");
		var row_data7 = $(this).attr("data7");
  ...
  html: row_data1 +
	...
	'<br>' + 
	row_data7  +
    '<br><a style="color:blue" href="#">Submit Info!</a> ',
...
<body>
	...
	$user = $_SESSION['nama'];
	$Cari="select * from activity_log where driver = '$user' and id_status = 'Recieved' order by input_date DESC";
	$Tampil = mysqli_query($Open, $Cari);
	    while (	$hasil = mysqli_fetch_array ($Tampil)) {
			...
			$pick_up_date = stripslashes ($hasil['pick_up_date']);
			$drop_off_date = stripslashes ($hasil['drop_off_date']);
			$reciever = stripslashes ($hasil['reciever']);
			$label_color = stripslashes ($hasil['label_color']);
		{
	?>
                                                                    <tr class='tblRows' data1='Nama Item:<?=$item_name?>.' 
																	data2='Tujuan:<?=$dest?>.' data3='Status:<?=$status?>.' 
																	data4='Input Date:<?=$input_date?>.' data5='Received Date:<?=$drop_off_date?>'
																	data6='Qty:<?=$qty?>' data7='Receiver:<?=$reciever?>'>
                                                                        <!--<td><label class="label label-<?=$label_color?>"><?=$status?></label></td>-->
																		<td><?=$id?></td>
                                                                        <td><h6><b><?=$item_name?></b><h6>
																		<span>- <?=$dest?></span><br>
																		<span>- <?=$input_date?></span></td>
																		<td><?=$qty?></td>
                                                                        <!--<td><?=$dest?></td>
                                                                        <td><?=$input_date?></td>
																		<td><?PHP 
																		$link_address = "form-drop-off.php?id=$id&item=$item_name";
																		$link_address2 = "#";
																		if($status == "Progress"){
							echo "<div align='left'><a type='button' class='btn btn-success' href='$link_address'>Drop Off</a></div>";
							
							} 
							else{
							echo "<div align='left'><a type='button' class='btn btn-primary' href='$link_address2'><i class='feather icon-navigation'></i></a></div>";
						}?>
</td>-->
                                                                    </tr>
																	<?php  
