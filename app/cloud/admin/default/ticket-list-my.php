...
$Open = mysqli_connect("127.0.0.1","root","root");
	...
	$Cari="select * from tickets where status not in ('Closed','Solved') and agent_dedicated = '$user' order by datecreate DESC";
	} 
	else{
	$Cari="select * from tickets where status not in ('Closed','Solved','Open') and agent_dedicated = '$user'  order by datecreate DESC";							
	}
